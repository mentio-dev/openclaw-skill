...
6. Classification: `relevance` 0 to 100 (`relevant` is true from 40 up), `sentiment` is `positive`, `neutral` or `negative`, `intents` are any of `buy_intent`, `question`, `complaint`, `praise`, `comparison`. It is `null` until the classifier has run.
...
| "Mark it done", "ignore it", "assign to Ana", "snooze until Monday", "note that we replied" | `PATCH /v1/mentions/{id}` with `status`, `assigneeId`, `snoozedUntil`, `note`; `null` clears | Assignees are workspace member user ids; if the user names a person you cannot resolve, ask for the id or skip the assignment. |
...
| "A daily digest at 9", "summary every morning" | `POST /v1/alerts` with `mode: "daily"`, `schedule: { hour, minute, timezone }` and `channelIds` | Ask for the timezone if unknown; `skipEmpty: true` skips quiet days. `POST /v1/alerts/{id}/run` sends the last 24 hours now. |
...
| "Too much noise", "it misses the real ones", "wrong relevance" | `GET /v1/company`, then `PATCH /v1/company` with a sharper `description` and `useCases` in the user's words | rules/account.md. The company profile is the classifier's context and the biggest lever on relevance; say so. |
...
- Confirm with the user before `DELETE /v1/keywords/{id}` (removes its matches), any other `DELETE`, `POST /v1/channels/{id}/rotate-secret` (the old secret stops verifying at once) and merging people.
...
- [rules/keywords.md](rules/keywords.md): track, pause, widen and remove keywords; what a keyword costs
- [rules/mentions.md](rules/mentions.md): every search filter, the mention shape, triage, CSV export
- [rules/people.md](rules/people.md): the people behind the mentions, tags, notes, mute, merge, saved segments
- [rules/alerts.md](rules/alerts.md): rules and channels, digests, webhooks and their signature, testing and the delivery log
- [rules/analytics.md](rules/analytics.md): the four reports and the window grammar
- [rules/account.md](rules/account.md): the company profile the classifier reads, API keys, health
