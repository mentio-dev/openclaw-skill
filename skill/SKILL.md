---
name: mentio
description: "Mentio social listening for developer platforms: track brand, competitor and topic keywords across Bluesky, Hacker News, Reddit, X, GitHub, Stack Overflow, DEV, YouTube, LinkedIn, TikTok and news; search and triage AI-classified mentions; set up Slack, Telegram, email and webhook alerts; read analytics and the people behind the mentions. Use when the user asks who is talking about their product, wants brand or competitor monitoring, or names Mentio."
...
5. Platforms are `bluesky`, `hackernews`, `github`, `stackoverflow`, `devto`, `reddit`, `x`, `youtube`, `news`, `linkedin`, `tiktok`. The field is always called `platform` (or `platforms` for a list).
...
7. A mention's `status` is the user's triage: `open` (untouched), `ignored` (hidden from the feed and every channel), `done` (handled). Ignored and done mentions are never delivered. A rule sends relevant mentions only (scored 40 and up) unless its filter's `minRelevance` goes under the line (0 for every scored match, noise included); email channels keep the line whatever the rule says.
...
| "What did keyword X cost?", "where does the money go?", "cost per keyword in September" | `GET /v1/usage/breakdown` | rules/account.md. Grouped `by` keyword, platform or day, over a `range` or a calendar `month`, with the window's totals; every keyword also carries this month's `stats.cost`. |
